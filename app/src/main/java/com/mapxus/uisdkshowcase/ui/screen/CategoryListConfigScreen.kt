...
@Composable
fun CategoryListConfigScreen(item: Item, onBack: () -> Unit) {
    val currentCategories = remember {
        mutableStateListOf(*(ConfigHolder.categoryListConfig?.toTypedArray() ?: emptyArray()))
    }
    ...
    ItemDetailFramework(
        ...
        onSaveClicked = {
            ConfigHolder.categoryListConfig = currentCategories.toList()
            scope.launch {
                snackbarHostState.showSnackbar("Category List Config saved")
            }
        }
    ) {
        FlowRow(
...
@Composable
private fun CategoryListConfigScreenPreview() {
    CategoryListConfigScreen(
        item = CategoryListConfig,
        onBack = {}
    )
}
