...
@Composable
fun FloorSelectorCategoriesScreen(item: Item, onBack: () -> Unit) {
    val currentCategories = remember {
        mutableStateListOf(*(ConfigHolder.floorSelectorCategories?.toTypedArray() ?: emptyArray()))
    }
    ...
    ItemDetailFramework(
        ...
        onSaveClicked = {
            ConfigHolder.floorSelectorCategories = currentCategories.toList()
            scope.launch {
                snackbarHostState.showSnackbar("Floor Selector Categories saved")
            }
        }
    ) {
        FlowRow(
...
@Composable
private fun FloorSelectorCategoriesScreenPreview() {
    FloorSelectorCategoriesScreen(
        item = FloorSelectorCategories,
        onBack = {}
    )
}
