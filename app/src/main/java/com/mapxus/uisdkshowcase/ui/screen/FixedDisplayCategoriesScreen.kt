...
@Composable
fun FixedDisplayCategoriesScreen(item: Item, onBack: () -> Unit) {
    val currentCategories = remember {
        mutableStateListOf(*(ConfigHolder.fixedDisplayCategories?.toTypedArray() ?: emptyArray()))
    }
    ...
    ItemDetailFramework(
        ...
        onSaveClicked = {
            ConfigHolder.fixedDisplayCategories = currentCategories.toList()
            scope.launch {
                snackbarHostState.showSnackbar("Fixed Display Categories saved")
            }
        }
    ) {
        FlowRow(
...
@Composable
private fun FixedDisplayCategoriesScreenPreview() {
    FixedDisplayCategoriesScreen(
        item = FixedDisplayCategories,
        onBack = {}
    )
}
