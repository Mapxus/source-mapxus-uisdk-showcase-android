...
fun GpsModeDisplayNameScreen(item: Item, onBack: () -> Unit) {
    var enable by remember { mutableStateOf(ConfigHolder.gpsModeDisplayName != null) }

    val defaultTextState =
        rememberTextFieldState(ConfigHolder.gpsModeDisplayName?.default ?: "")
    val enTextState = rememberTextFieldState(ConfigHolder.gpsModeDisplayName?.en ?: "")
    val zhHansTextState =
        rememberTextFieldState(ConfigHolder.gpsModeDisplayName?.zhHans ?: "")
    val zhHantTextState =
        rememberTextFieldState(ConfigHolder.gpsModeDisplayName?.zhHant ?: "")
    val zhHanTWTextState =
        rememberTextFieldState(ConfigHolder.gpsModeDisplayName?.zhHantTW ?: "")
    val jaTextState = rememberTextFieldState(ConfigHolder.gpsModeDisplayName?.ja ?: "")
    val koTextState = rememberTextFieldState(ConfigHolder.gpsModeDisplayName?.ko ?: "")
    val arTextState = rememberTextFieldState(ConfigHolder.gpsModeDisplayName?.ar ?: "")
    ...
    ItemDetailFramework(
        ...
        onSaveClicked = {
            ConfigHolder.gpsModeDisplayName = if (enable) {
                StringsWithLanguage(
            ...
            scope.launch {
                snackbarHostState.showSnackbar("GPS Mode Display Name saved")
            }
        }
    ) {
        ...
        TextCheckBox(
            text = "Modify GPS Mode Display Name",
...
@Composable
private fun GpsModeDisplayNameScreenPreview() {
    GpsModeDisplayNameScreen(
        item = GpsModeDisplayName,
        onBack = {}
    )
}
